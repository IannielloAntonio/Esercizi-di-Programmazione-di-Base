...
\]
	...
	Scrivere un programma che riceve in ingresso una sequenza di N numeri
	interi con N scelto dall'utente. 
	Terminato l’inserimento della sequenza di numeri, l’utente inserisce un valore di riferimento. Il programma deve indicare se tale
	valore di riferimento è contenuto nel vettore.
	
	
	\subsubsection{Shift di una Lista}
...
\begin{itemize}
	\item Stampare la lista
	\item Eseguire uno spostamento (shift) a sinistra di una posizione del contenuto del vettore.
	Pertanto ogni elemento del vettore deve assumere il valore dell’elemento immediatamente successivo all’interno del vettore. L’elemento di indice N-1 deve assumere il
	valore zero.
	Ad esempio: \\ 
	la lista 1 10 15 18 \\
	deve diventare 10 15 18 0
	\item Il programma visualizza il vettore ottenuto.
\end{itemize}
	
	\subsubsection{Shift di una Lista 2.0}
	...
	\begin{itemize}
		...
		\item Eseguire uno spostamento (shift) a sinistra di una posizione del contenuto del vettore.
		Pertanto ogni elemento del vettore deve assumere il valore dell’elemento immediatamente successivo all’interno del vettore. L’elemento di indice $N-1$ deve assumere il
		valore dell'elemento di indice $0$.
		Ad esempio: \\ 
		la lista 1 10 15 18 \\
	...
	\subsubsection{Conta le occorrenze}
	Scrivere un programma in Python che conti quante volte un determinato elemento compare all'interno di una lista.
	
	Il programma deve:
	\begin{itemize}
		\item chiedere all’utente di inserire una lista di numeri interi separati da spazio;
		\item chiedere all’utente di inserire un numero da cercare nella lista;
		\item contare il numero di occorrenze del valore inserito;
		\item stampare a schermo il risultato.
	\end{itemize}
	...
	\subsubsection{Conta l'elemento che si ripete più volte}
	...
	\begin{itemize}
		\item chiedere all’utente di inserire una lista di numeri interi separati da spazio;
		\item analizzare la lista e determinare l’elemento che si ripete più volte;
		\item stampare a schermo l’elemento trovato e il numero di occorrenze.
	\end{itemize}
	...
	\subsubsection{Calcolatrice}
	Realizzare un programma in Python che simuli il comportamento di una calcolatrice.
	Il programma userà diverse funzioni il cui scopo sarà:
	\begin{itemize}
		\item somma
		\item differenza
		\item prodotto
	...
	\subsubsection{Saluto personale}
	Scrivere una funzione \texttt{saluta} che stampi a video: "Ciao, \texttt{nome}!".  
	Chiedere all'utente di inserire il proprio nome e chiamare la funzione.
	
	\subsubsection{Numero pari o dispari}
	Scrivere una funzione \texttt{pari} che ritorni \texttt{True} se il numero \texttt{n} è pari, \texttt{False} altrimenti.  
	Chiedere all'utente di inserire un numero e stampare se è pari o dispari.
	
	\subsubsection{Calcolo del quadrato}
	Scrivere una funzione \texttt{quadrato} che ritorni il quadrato del numero \texttt{x}.  
	Testare la funzione con numeri scelti dall'utente.
	
	\subsubsection{Calcolo del cubo}
	Scrivere una funzione \texttt{cubo} che ritorni il cubo del numero \texttt{x}.  
	Chiedere all'utente di inserire un numero e stampare il risultato.
	
	\subsubsection{Media di due numeri}
	Scrivere una funzione \texttt{media} che ritorni la media dei due numeri.  
	Chiedere all'utente di inserire due numeri e stampare la media.
	
	\subsubsection{Conversione da Celsius a Fahrenheit}
	Scrivere una funzione \texttt{celsius\_to\_fahrenheit} che ritorni la temperatura in gradi Fahrenheit corrispondente a \texttt{c} gradi Celsius usando la formula:  
	\[ F = \frac{9}{5} \cdot C + 32 \]  
	Chiedere all'utente di inserire una temperatura in Celsius e stampare il risultato.
	
	\subsubsection{Calcolo del perimetro di un rettangolo}
	Scrivere una funzione \texttt{perimetro} che ritorni il perimetro di un rettangolo dato lunghezza \texttt{l} e altezza \texttt{h}.  
	Chiedere all'utente di inserire lunghezza e altezza e stampare il perimetro.
	
	\subsubsection{Velocità media}
	Scrivere una funzione \texttt{velocita\_media} che calcoli e ritorni la velocità media di un corpo dato lo spazio percorso \texttt{d} (in metri) e il tempo impiegato \texttt{t} (in secondi).  
	Chiedere all'utente di inserire spazio e tempo e stampare la velocità media in m/s.
	
	\subsubsection{Energia cinetica}
	Scrivere una funzione \texttt{energia\_cinetica} che calcoli e ritorni l'energia cinetica di un corpo con massa \texttt{m} (in kg) e velocità \texttt{v} (in m/s), usando la formula:  
	\[
	E_k = \frac{1}{2} m v^2
	\]  
	...
	Scrivere una funzione \texttt{forza\_gravitazionale(m1, m2, r)} che calcoli e ritorni la forza di attrazione gravitazionale tra due corpi di massa \texttt{m1} e \texttt{m2} (in kg) distanti \texttt{r} metri, usando la legge di Newton:  
	\[
	F = G \frac{m_1 m_2}{r^2}
	\]  
	dove \( G = 6.674 \times 10^{-11} \, \text{N·m²/kg²} \).  
	Chiedere all'utente di inserire le masse e la distanza e stampare la forza risultante.
	
	\subsubsection{Somma dei primi N numeri}
	Scrivere una funzione \texttt{somma\_primi} che calcoli e ritorni la somma dei primi \texttt{N} numeri interi positivi usando un ciclo \texttt{for}.  
	Chiedere all'utente di inserire \texttt{N} e stampare il risultato.
	
	\subsubsection{Tabellina di un numero}
	Scrivere una funzione \texttt{tabellina} che stampi la tabellina del numero \texttt{n} da 1 a 10 usando un ciclo \texttt{for}.  
	Chiedere all'utente di inserire il numero e chiamare la funzione.
	
	\subsubsection{Prodotto di una lista}
	Scrivere una funzione \texttt{prodotto\_lista} che calcoli e ritorni il prodotto di tutti i numeri presenti in una lista, usando un ciclo \texttt{for}.  
	Chiedere all'utente di inserire 5 numeri e stampare il prodotto.
	
	\subsubsection{Contatore di numeri pari}
	Scrivere una funzione \texttt{conta\_pari} che conti quanti numeri pari ci sono in una lista usando un ciclo \texttt{for}.  
	Chiedere all'utente di inserire 5 numeri e stampare il numero di valori pari.
	
	\subsubsection{Fattoriale di un numero}
	Scrivere una funzione \texttt{fattoriale} che calcoli e ritorni il fattoriale di \texttt{n} usando un ciclo \texttt{for}.  
	Chiedere all'utente di inserire il numero e stampare il fattoriale.
	
	\subsubsection{Somma dei numeri dispari in una lista}
	Scrivere una funzione \texttt{somma\_dispari} che calcoli e ritorni la somma di tutti i numeri dispari presenti in una lista usando un ciclo \texttt{for} e un controllo \texttt{if}.  
	Chiedere all'utente di inserire 5 numeri e stampare il risultato.
	
	\subsubsection{Trova il massimo in una lista}
	Scrivere una funzione \texttt{massimo} che trovi e ritorni il numero più grande in una lista senza usare funzioni predefinite come \texttt{max()}.  
	Usare un ciclo \texttt{for} per confrontare gli elementi.
	
	\subsubsection{Conta vocali in una parola}
	Scrivere una funzione \texttt{conta\_vocali} che conti quante vocali (\texttt{a, e, i, o, u}) ci sono in una parola inserita dall'utente.  
	
	\subsubsection{Filtra numeri maggiori di N}
	Scrivere una funzione \texttt{filtra\_maggiori} che ritorni una nuova lista contenente solo i numeri maggiori di \texttt{N} presenti nella lista originale.  
	Usare un ciclo \texttt{for} e un \texttt{if} per il filtro.
	
	\subsubsection{Lista dei quadrati}
	Scrivere una funzione \texttt{quadrati(lista)} che ritorni una nuova lista contenente il quadrato di ogni numero della lista originale.  
	Usare un ciclo \texttt{for} per costruire la nuova lista.
	
	\subsubsection{Statistiche su una lista}
	Scrivere un programma Python con queste funzioni:
	\begin{itemize}
		\item \texttt{somma}: ritorna la somma dei numeri
		\item \texttt{media)}: ritorna la media dei numeri
		\item \texttt{massimo}: ritorna il numero più grande
		\item \texttt{minimo}: ritorna il numero più piccolo
	\end{itemize}
	Chiedere all'utente di inserire una lista di numeri e la statistica che vuole visualizzare.
	
	\subsubsection{Gestione voti studenti}
	Scrivere un programma Python con queste funzioni:
	\begin{itemize}
		\item \texttt{media\_voti}: ritorna la media dei voti
		\item \texttt{voti\_sufficiente}: ritorna il numero di voti maggiori o uguali a 6
		\item \texttt{voti\_insufficiente}: ritorna il numero di voti minori di 6
	\end{itemize}
	Chiedere all'utente di inserire la lista dei voti di una classe e stampare i risultati usando le funzioni.
	
	\subsubsection{Gioco: Indovina il numero}
	Realizzare un gioco in Python in cui il computer sceglie un numero casuale tra 1 e 100 e l'utente deve indovinarlo.  
	Il programma deve utilizzare più funzioni:
	
	\begin{itemize}
		\item \texttt{genera\_numero}: genera e ritorna un numero casuale tra 1 e 100
		\item \texttt{chiedi\_numero}: chiede all'utente di inserire un numero e lo ritorna
		\item \texttt{controlla\_numero}: confronta il numero inserito con il numero segreto e stampa un messaggio:
		\begin{itemize}
			\item "Troppo alto" se il numero utente è maggiore
			\item "Troppo basso" se il numero utente è minore
			\item "Esatto!" se il numero utente è corretto
		\end{itemize}
	\end{itemize}
	
	Il programma principale deve ripetere la richiesta finché l’utente non indovina il numero segreto.
	
	\subsubsection{Gioco: Caccia al cibo}
	Realizzare un gioco in Python in cui un giocatore si muove su una griglia 10x10.  
	Lo scopo del gioco è trovare il cibo evitando le bombe, posizionate casualmente nella griglia. Il giocatore non sa in anticipo quali celle contengono cibo o bombe. Ogni mossa sicura aumenta il punteggio del giocatore di 10 punti.
	
	Il programma deve utilizzare più funzioni:
	
	\begin{itemize}
		\item \texttt{crea\_griglia}: crea e ritorna una griglia 5x5 vuota e posiziona casualmente:
		\begin{itemize}
			\item 1 cibo
			\item 3 bombe
		\end{itemize}
		\item \texttt{stampa\_griglia}: stampa la griglia mostrando solo la posizione del giocatore con un simbolo "P", mentre le altre celle rimangono nascoste
		\item \texttt{muovi\_giocatore}: riceve la posizione corrente del giocatore e la direzione scelta dall'utente ("su", "giù", "sinistra", "destra") e ritorna la nuova posizione, gestendo i bordi della griglia
		\item \texttt{controlla\_cella}: controlla il contenuto della cella in cui si trova il giocatore:
		\begin{itemize}
			\item Se è cibo, stampa "Hai trovato il cibo! Hai vinto!" e termina il gioco
			\item Se è una bomba, stampa "Hai calpestato una bomba! Hai perso!" e termina il gioco
			\item Altrimenti continua e aggiunge 10 punti al punteggio
		\end{itemize}
		\item \texttt{stampa\_punteggio}: stampa il punteggio del giocatore
	\end{itemize}
	
	Il programma principale deve:
	\begin{itemize}
		\item Creare la griglia e inizializzare la posizione del giocatore in alto a sinistra
		\item Inizializzare il punteggio a 0
		\item Ripetere:
		\begin{itemize}
			\item Stampare la griglia
			\item Chiedere all'utente la direzione da muovere
			\item Aggiornare la posizione del giocatore
			\item Controllare il contenuto della cella
			\item Aggiornare e stampare il punteggio
		\end{itemize}
...
for i in range(N):
	...
	lista.append(numero) # la funzione append permette di inserire la variabile numero dopo l'ultimo elemento
...
\begin{lstlisting}
	...
	def saluta(nome):
		# Stampa il messaggio di saluto usando il nome passato come parametro
		print("Ciao,", nome + "!")
...
\begin{lstlisting}
	...
	celsius = float(input("Inserisci la temperatura in Celsius: "))
	print("Temperatura in Fahrenheit:", celsius_to_fahrenheit(celsius))
\end{lstlisting}
...
\begin{lstlisting}
	...
	print("La velocita' media e':", velocita_media(distanza, tempo), "m/s")
\end{lstlisting}
...
\begin{lstlisting}
	...
	print("Energia cinetica:", energia_cinetica(massa, velocita), "J")
\end{lstlisting}
...
\begin{lstlisting}
	...
	print("Forza gravitazionale:", forza_gravitazionale(m1, m2, r), "N")
\end{lstlisting}
...
\begin{lstlisting}
	...
	print("Numeri maggiori di N:", filtra_maggiori(lista, N))
\end{lstlisting}
